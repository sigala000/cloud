...
def Deploy():
    # args = json.load(open('config_ova.json'))
    ##THIS IS WHERE WE LOAD OUR JSON FILE
    sig = json.load(open('config_ova_cluster.json'))
    for key in sig.keys():

        # print(args)
        si = service_instance.connect(sig[key])

        if sig[key]['datacenter_name']:
            datacenter = get_dc(si, sig[key]['datacenter_name'])
        else:
            datacenter = si.content.rootFolder.childEntity[0]

        if sig[key]['resource_pool']:
            resource_pool = get_rp(si, datacenter, sig[key]['resource_pool'])
        else:
            resource_pool = get_largest_free_rp(si, datacenter)

        if sig[key]['datastore_name']:
            datastore = get_ds(datacenter, sig[key]['datastore_name'])
        else:
            datastore = get_largest_free_ds(datacenter)

        ovf_handle = OvfHandler(sig[key]['ova_path'])
        ...
        # Deply vm
        ovf_handle.upload_disks(lease, sig[key]['host'])
        
        print(f"Deploying {sig[key]['instance_nbr']} instances...")
        
        # for x in range(sig[key]['instance_nbr']-1):
        #     print(f"Deploying vm_{x+1} ...")
...
def main():
    # args = json.load(open('config_ova_cluster.json'))
    ...
    # print(args)
        ...
        for x in range(sig[key]['instance_nbr']-1):
            print(f"Deploying vm_{x+1} ...")
            Deploy() ## HERE WE DEFINE THE NUMBER OF INSTANCES WE WANT OUR VM TO HAVE
        exit()
            # content = si.RetrieveContent()
            # ovf_handle.upload_disks(lease, sig[key]['host'])
